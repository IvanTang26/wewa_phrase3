class WhackAMole {
    ...
    handleHoleHit(hole) {
        ...
        if (this.firstHitTime === null) {
            this.firstHitTime = ((60000 - this.timer) / 1000).toFixed(3);
        }
    ...
    spawnMole(hole, type) {
        ...
        if (type === 'yellow') {
            mole.src = '6.ball_character/yellow_01/yellow_01_rise2.gif';
            mole.style.width = '100%';
    ...
    async endGame() {
        ...
        // Update start time with correct format
        const startTime = this.screens.finalScore.querySelector('.start-time');
        if (this.firstHitTime !== null) {
            startTime.textContent = (this.firstHitTime / 1000).toFixed(3).padStart(6, '0');
        } else {
